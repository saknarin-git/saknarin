  ...
  if (reportType === 'working-day') {
    ...
    const normalizedRows = (contractResult.data ?? [])
      .filter((contract) => {
        ...
        const priorPrincipalTotal = roundMoney(priorPayments.reduce((sum, payment) => sum + Number(payment.principal_paid ?? 0), 0));
        const sameDayPrincipalTotal = roundMoney(sameDayPayments.reduce((sum, payment) => sum + Number(payment.principal_paid ?? 0), 0));
        const openingBalance = roundMoney(Math.max(0, loanAmount - priorPrincipalTotal));
        const closingBalance = roundMoney(Math.max(0, openingBalance - sameDayPrincipalTotal));
        if (!contractDate) {
          return false;
        }
      ...
      .map((contract) => {
        ...
        const priorPrincipalTotal = roundMoney(priorPayments.reduce((sum, payment) => sum + Number(payment.principal_paid ?? 0), 0));
        const openingBalance = roundMoney(Math.max(0, loanAmount - priorPrincipalTotal));
        const remainingBalance = roundMoney(Math.max(0, openingBalance - normalPrincipalAmount - settlementPrincipalAmount));
